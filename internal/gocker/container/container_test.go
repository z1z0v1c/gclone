...
func TestUserNamespaceIsolation(t *testing.T) {
	...
	// The process should NOT be running as root (UID 0) on the host
	if sleepUID == 0 {
	...
	// The process should be running as the host user (user namespace mapping)
	if sleepUID != hostUID {
	...
	// The process should be running with the host user's GID
	if sleepGID != hostGID {
	...
	// The username should match the host user
